                ...
                if ($user_exists > 0) {
                    ...
                    foreach ($get_user_details as $value) {
                        ...
                        if ($count_msg == 18) {
                            ...
                            $serial_no = @$explode_msg[2];
                            $f_name = @$exploded_msg[3]; //FIRST NAME
                            $m_name = @$exploded_msg[4]; //MIDDLE NAME
                            $l_name = @$exploded_msg[5]; //LAST NAME
                            $dob = @$exploded_msg[6]; //DATE OF BIRTH
                            $national_id = @$explode_msg[7]; //NATIONAL ID OR PASSOPRT NO
                            $gender = @$exploded_msg[8]; //GEDNER
                            $marital = @$exploded_msg[9]; //MARITAL STATUS
                            $condition = @$exploded_msg[10]; //CONDITION
                            $enrollment_date = @$exploded_msg[11]; //ENROLLMENT DATE
                            $art_start_date = @$exploded_msg[12]; //ART START DATE
                            $phone_no = @$exploded_msg[13]; //PHONE NUMBER
                            $sms_enable = @$exploded_msg[14]; //SMS ENABLE 
                            $language = @$exploded_msg[15]; //LANGUAGE
                                                ...
                                                if ($check_client_existence > 0) {
                                                    ...
                                                    if ($this->db->trans_status() === FALSE) {
                                                        ...
                                                        if ($this->db->trans_status() === FALSE) {
                                                            ...
                                                            $send_text = $this->send_message($source, $destination, $msg);
                                                        }
                                                    }
                                                } else {
